...
class TextEncoder(nn.Module):
    ...
    def forward(self, x, bias=None):
        if  (x == 0).nonzero().size():
            return self.forward_mixed_len_batches(x, bias)
        else:
            return self.forward_same_len_batches(x, bias)

    def forward_same_len_batches(self, x, bias=None):
        # Fetch embeddings
        embs = self.emb(x)
        if bias is not None:
            embs += bias
    ...
    def forward_mixed_len_batches(self, x, bias=None):
