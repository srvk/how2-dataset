...
from ..layers import TextEncoder, ConditionalDecoder, FF
...
class NMT(nn.Module):
    ...
    def encode(self, batch, **kwargs):
        ...
        """
        if self.opts.model['adaptation']:
            # NOTE: The avg pooled features only converge when scaled down
            # with 0.1. Otherwise, some sort of catastrophic forgetting happens.
            visual_shift = self.vis_proj(batch['feats'] * 0.1)
        else:
            visual_shift = None
        return {str(self.sl): self.enc(batch[self.sl], visual_shift)}
